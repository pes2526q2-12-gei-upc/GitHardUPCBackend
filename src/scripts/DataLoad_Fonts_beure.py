...
log_filename = os.path.join(LOGS_DIR, f"{datetime.now().strftime('%Y-%m-%d_%H-%M-%S')}_dataload-fonts-beure.log")
...
def main():
    logging.info("---------- Actualitzant dades de l'API de Fonts de Beure ----------")
    config = get_config()

    url_api = config.get("opendata.fonts_beure.url")
    t_fonts_beure = config.get("db.table.fonts_beure")
    ...
    if not all([db_user, db_pass, db_url_jdbc, url_api, t_fonts_beure]):
    ...
    logging.info("---------- Dades de l'API de Fonts de Beure actualitzades ----------")
