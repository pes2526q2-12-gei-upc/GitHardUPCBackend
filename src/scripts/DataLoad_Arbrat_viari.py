...
log_filename = os.path.join(LOGS_DIR, f"{datetime.now().strftime('%Y-%m-%d_%H-%M-%S')}_dataload-arbrat-viari.log")
...
def main():
    logging.info("---------- Actualitzant dades de l'API d'Arbrat Viari ----------")
    config = get_config()

    url_api = config.get("opendata.arbrat_viari.url")
    t_arbrat_viari = config.get("db.table.arbrat_viari")
    ...
    if not all([db_user, db_pass, db_url_jdbc, url_api, t_arbrat_viari]):
