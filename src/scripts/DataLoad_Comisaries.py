...
log_filename = os.path.join(LOGS_DIR, f"{datetime.now().strftime('%Y-%m-%d_%H-%M-%S')}_dataload-comissaries.log")
...
def main():
    logging.info("---------- Actualitzant dades de l'API de Comissaries ----------")
    config = get_config()

    url_api = config.get("opendata.comissaries.url")
    t_comissaries = config.get("db.table.comissaries")
    ...
    if not all([db_user, db_pass, db_url_jdbc, url_api, t_comissaries]):
    ...
    try:
        ...
        # Llegim el CSV des de la cadena de text ja decodificada
        # Fem servir sep=',' perquè ara ja veiem que les capçaleres estan separades per comes
    ...
    logging.info("---------- Dades de l'API de Comissaries actualitzades ----------")
