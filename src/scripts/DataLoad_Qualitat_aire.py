...
def main():
    logging.info("---------- Actualitzant dades de l'API de Qualitat d'Aire ----------")
    config = get_config()

    url_api = config.get("opendata.qualitat_aire.url")
    ...
    if not all([db_user, db_pass, db_url_jdbc, url_api, t_qualitat_aire]):
    ...
    try:
        ...
        logging.info(f"Pujant {len(df)} files a la taula '{t_qualitat_aire}'...")
        with engine.begin() as conn:
            conn.execute(text(f'DROP TABLE IF EXISTS "{t_qualitat_aire}" CASCADE;'))

        df.to_sql(t_qualitat_aire, engine, if_exists='replace', index=False)
        logging.info(f"ÈXIT: Taula '{t_qualitat_aire}' creada amb {df.shape[1]} columnes.")

    except Exception as e:
        logging.error(f"Error en el processament: {e}")

    logging.info("---------- Dades de l'API de Qualitat d'Aire actualitzades ----------")
