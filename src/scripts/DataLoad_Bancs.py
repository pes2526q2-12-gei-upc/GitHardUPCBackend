...
log_filename = os.path.join(LOGS_DIR, f"{datetime.now().strftime('%Y-%m-%d_%H-%M-%S')}_dataload-bancs.log")
...
def main():
    logging.info("---------- Actualitzant dades de l'API de Bancs ----------")
    config = get_config()

    url_api = config.get("opendata.bancs.url")
    t_bancs = config.get("db.table.bancs")
    ...
    if not all([db_user, db_pass, db_url_jdbc, url_api, t_bancs]):
    ...
    try:
        ...
        logging.info(f"Pujant {len(df)} files a la taula '{t_bancs}'...")
        with engine.begin() as conn:
            conn.execute(text(f'TRUNCATE TABLE "{t_bancs}" RESTART IDENTITY CASCADE;'))

        df.to_sql(t_bancs, engine, if_exists='append', index=False)
        logging.info(f"ÈXIT: Taula '{t_bancs}' creada amb {df.shape[1]} columnes.")

    except Exception as e:
        logging.error(f"Error en el processament: {e}")

    logging.info("---------- Dades de l'API de Bancs actualitzades ----------")
