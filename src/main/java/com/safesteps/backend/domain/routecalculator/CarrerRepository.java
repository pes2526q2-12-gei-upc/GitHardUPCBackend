...
public interface CarrerRepository extends JpaRepository<Carrer, Long> {
         ...
         * Utilitza la funcio pgr_dijkstra de PostGIS per a calcular la ruta mes curta
         * entre dos nodes.
         * Tambe utilitza una vista (v_trams_nodes) que conté les dades dels carrers
         * (id, source, target, longitud)
         * creuant els carrers amb els nodes de la BD.
         */
        @Query(value = "SELECT di.node AS node, di.edge AS edge, di.seq AS seq, COALESCE(v.longitud, 0.0) AS cost " +
                        "FROM pgr_dijkstra(" +
                        ...
                        +
                        "              + (' || :#{#filtre.infraccions} || ' * GREATEST(0, 1.0 - COALESCE(cnt_infraccions, 0.0))) "
                        + // NOELTINC
                        ...
                        // LEAST(1, cnt_incidents) evita que m\u00faltiples incid\u00e8ncies amplifiquin
                        // el cost m\u00e9s enll\u00e0 del pes definit.
        ...
        /*
         * Query per a trobar les coordenades (latitud i longitud) a partir dels
         * identificadors dels nodes.
         * Utilitza les funcions de PostGIS per a transformar les coordenades dels nodes
         * de la BD (UTM) a coordenades GPS (lat i long.) (WGS84).
         */
        @Query(value = "SELECT ST_X(geom_gps) AS lon, ST_Y(geom_gps) AS lat " +
                        "FROM (" +
                        "  SELECT t.seq, " +
        ...
        /*
         * Query per a trobar el node mes proper a unes coordenades donades (latitud i
         * longitud).
         * Utilitza les funcions de PostGIS per a calcular la distancia entre les
         * coordenades donades i les coordenades dels nodes de la BD.
         * Donada una entrada en lat. i long. retorna el FID del node mes proper a
         * aquestes coordenades.
         * Com la BD esta en UTM (EPSG:25831) i les coordenades d'entrada estan en WGS84
         * (EPSG:4326),
         * es necessari transformar les coordenades d'entrada a UTM abans de calcular la
         * distancia.
         */
         ...
         */
        @Query(value = "WITH route_geom AS ( " +
                        ...
                        "WHERE ST_DWithin(ST_SetSRID(ST_MakePoint(c.geo_epgs_25831_x, c.geo_epgs_25831_y), 25831), rg.geom, :radi)", nativeQuery = true)
        ...
        @Query(value = "WITH route_geom AS ( " +
                        ...
                        "WHERE ST_DWithin(ST_SetSRID(ST_MakePoint(f.x_etrs89, f.y_etrs89), 25831), rg.geom, :radi)", nativeQuery = true)
         ...
         * Forcem el nom a 'Càmera de seguretat' ja que no hi ha una descripció amigable
         * a la BD.
         */
        @Query(value = "WITH route_geom AS ( " +
                        "  SELECT ST_MakeLine(ST_SetSRID(ST_MakePoint(n.\"Coord_X\", n.\"Coord_Y\"), 25831) ORDER BY t.seq) as geom "
         ...
         * Utilitza les coordenades ETRS89 projectades a 25831 per a distàncies en
         * metres.
         */
        @Query(value = "WITH route_geom AS ( " +
                        "  SELECT ST_MakeLine(ST_SetSRID(ST_MakePoint(n.\"Coord_X\", n.\"Coord_Y\"), 25831) ORDER BY t.seq) as geom "
                        ...
                        "WHERE ST_DWithin(ST_SetSRID(ST_MakePoint(e.geo_epgs_25831_x, e.geo_epgs_25831_y), 25831), rg.geom, :radi)", nativeQuery = true)
         ...
         * Radi EXTREMADAMENT curt (15m) per evitar col·lapsar el JSON amb milers
         * d'arbres.
         */
        @Query(value = "WITH route_geom AS ( " +
                        "  SELECT ST_MakeLine(ST_SetSRID(ST_MakePoint(n.\"Coord_X\", n.\"Coord_Y\"), 25831) ORDER BY t.seq) as geom "
                        ...
                        "WHERE ST_DWithin(ST_SetSRID(ST_MakePoint(a.x_etrs89, a.y_etrs89), 25831), rg.geom, :radi)", nativeQuery = true)
         ...
         * Radi de 15m per mantenir el rendiment i donar només l'ombra real del
         * trajecte.
         */
        @Query(value = "WITH route_geom AS ( " +
                        "  SELECT ST_MakeLine(ST_SetSRID(ST_MakePoint(n.\"Coord_X\", n.\"Coord_Y\"), 25831) ORDER BY t.seq) as geom "
        ...
        @Query(value = "WITH route_geom AS ( " +
                        ...
                        "  JOIN bcn_grafvial_nodes n ON n.\"FID\" = t.fid " +
                        ") " +
                        "SELECT 'Refugi climàtic' as name, r.geo_epgs_4326_lat as lat, r.geo_epgs_4326_lon as lon " +
                        "FROM bcn_refugis_climatics r, route_geom rg " +
                        "WHERE ST_DWithin(ST_Transform(ST_SetSRID(ST_MakePoint(r.geo_epgs_4326_lon, r.geo_epgs_4326_lat), 4326), 25831), rg.geom, :radi)", nativeQuery = true)
        List<PoiDBProjection> findRefugisNearRoute(@Param("fids") Long[] fids, @Param("radi") Double radiMetres);
        ...
        @Query(value = "WITH route_geom AS ( " +
                        ...
                        "  JOIN bcn_grafvial_nodes n ON n.\"FID\" = t.fid " +
                        ") " +
                        "SELECT i.type as name, ST_Y(i.location) as lat, ST_X(i.location) as lon " +
                        "FROM incidents i, route_geom rg " +
                        "WHERE i.status = 'ACCEPTED' AND ST_DWithin(ST_Transform(i.location, 25831), rg.geom, :radi)", nativeQuery = true)
        List<PoiDBProjection> findIncidentsNearRoute(@Param("fids") Long[] fids, @Param("radi") Double radiMetres);

        /*
         * Avaluació externa de rutes: Agafem un LINESTRING (wkt_line), mirem quins
         * trams
         * de carrer intersequen a menys de 20m, i fem la mitjana de tots els seus
         * scores.
         */
        @Query(value = "SELECT " +
                        ...
                        "AND ST_DWithin(geom, ST_Transform(ST_GeomFromText(:wktLine, 4326), 25831), 20)", nativeQuery = true)
