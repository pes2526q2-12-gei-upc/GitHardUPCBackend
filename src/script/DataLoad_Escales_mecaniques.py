...
log_filename = os.path.join(LOGS_DIR, f"{datetime.now().strftime('%Y-%m-%d_%H-%M-%S')}_dataload-escales-mecaniques.log")
...
def main():
    logging.info("---------- Actualitzant dades de l'API d'Escales Mecàniques ----------")
    config = get_config()

    url_api = config.get("opendata.escales_mecaniques.url")
    t_escales_mecaniques = config.get("db.table.escales_mecaniques")
    ...
    if not all([db_user, db_pass, db_url_jdbc, url_api, t_escales_mecaniques]):
    ...
    logging.info("---------- Dades de l'API d'Escales Mecàniques actualitzades ----------")
